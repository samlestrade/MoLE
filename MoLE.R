world=list(
	#general
	deathAge=1000,	#if NA, eternal life
	...
	nNouns=999,	#number of nouns in the lexicon
	nVerbs=499,	#number of verbs in the lexicon
	...
	oddsNewExternal=1/4,	#odds for a non common-ground element to enter as external argument of one of the events in the situation (element will be added to the common ground if discussed)
	oddsNewInternal=1/4,	#odds for a non common-ground element to enter as internal argument of one of the events in the situation (element will be added to the common ground if discussed)
	...
	roleTopicality=c(50, 50, 1),	#preference for actor, undergoer, and event to become the topic of the utterance
	personTopicality=c(2, 1, 2, 2),	#preference for speaker, addressee, animate third person, and inanimate third person respectively to be the topic of the utterance and participant in a situation (based on Dahl's A and A numbers)
	#production/interpretation
	wordOrder=F,	#should agents try to use word-order generalizations to mark/determine roles?
	...
	candidateScoring='all',	#In what order should candidates be considered (first one to suffice is selected): by activation, frequency, match, economy, collostruction, all. If all: values are rescaled to 0-1 range and summed (after weighing)
	frequency='relative',	#as argument, or role or index marker vs absolute
	activationImpact=1/5,	#if candidateOrdering=='all', how should activation be weighed with respect to match? activation subsumes (and replaces) frequency and recency impact=1:equally, impact=below 1: impact times less important, impact=above 1: impact times more important.
	frequencyImpact=1/10,	#if candidateOrdering=='all2', how should frequency be weighed with respect to match? impact=1:equally, impact=below 1: impact times less important, impact=above 1: impact times more important.
	collostructionImpact=1/100,	#if candidateOrdering=='all' and for finding index referent, how should collostruction frequency be weighed with respect to match? impact=1:equally, impact=below 1: impact times less important, impact=above 1: impact times more important.
	recencyImpact=1/10, 	#if candidateOrdering=='all', how should recency be weighed with respect to match? impact=1:equally, impact=below 1: impact times less important, impact=above 1: impact times more important.
	...
	economyImpact=1/100,	#if candidateOrdering=='all', how should economy be weighed with respect to match? impact=1:equally, impact=below 1: impact times less important, impact=above 1: impact times more important.
	...
	agentFirst=F, 	#is AgentFirst used as a strategy?
	topicFirst=F, #should topic be put in first position?
	topicCopy=T,	#should a (pronominal) copy of the topic be put adjacent to the verb (a la Givon; only applies if TopicFirst==T)? 
	orderAgentFirstTopicFirst='TA', 	#if both AgentFirst and TopicFirst are true, which of the two has priority? TA, AT, random
	#production
	...
	formBlocking=T,	#should agent reduce forms if this would lead to ambiguity?
	...
	desemanticizationPower=1.5,	#Development of thresholds for subsequent dimensions to be removed. 1 for linear development. Best between 1 and 2? The lower, the more difficult to desemanticize, as the threshold develops linearly to the same target (desemanticizationCeiling)
	...
	fusionThreshold=.03, 	#proportion of number of utterances in which a combination has to occur before it is fused
	#data management
...
FIRSTSPEAKER=function(){
	...
	flagCollostructions=data.frame(marker=1, N=1, frequency=0, stringsAsFactors=F); flagCollostructions=flagCollostructions[-1,]
	indexCollostructions=data.frame(marker=1, N=1, frequency=0, stringsAsFactors=F); indexCollostructions=indexCollostructions[-1,]
	roleCorrelation=data.frame(role=c('actor','undergoer'),external=0, internal=0, stringsAsFactors=F)
	position=data.frame(role=c('external','internal', 'actor', 'undergoer'),first=0, success=0, oddsSuccess=0, stringsAsFactors=F)
	index=data.frame(role=rep(c('external','internal', 'actor', 'undergoer'), each=3), person=rep(1:3, 4), yes=0, no=4, stringsAsFactors=F)	#cf. Yang's log operations later on. Only with minimally 4 exceptions, yang threshold is minority indeed
	values=vector(); for(i in 1:length(distinctions)){values=c(values, seq(0,1,length.out=distinctions[i]))}
	flag=list(
		person=data.frame(role=rep(c('external','internal', 'actor', 'undergoer'), each=3), person=rep(1:3, 4), yes=0, no=4, stringsAsFactors=F), 
		external=data.frame(dimension=rep(1:length(distinctions), distinctions), value=values, yes=0, no=4, stringsAsFactors=F),
		internal=data.frame(dimension=rep(1:length(distinctions), distinctions), value=values, yes=0, no=4, stringsAsFactors=F),
	...
	wordOrder=data.frame(order=c('SVO','SOV', 'VSO', 'VOS', 'OSV', 'OVS'), freq=0, success=0, oddsSuccess=0, stringsAsFactors=F)
	commonGround=vector()
	list(age=0, generation=1, fertile='yes', semupdate=0, verbs=verbs, nouns=nouns, usageHistory=usageHistory, commonGround=commonGround, collostructions=list(SV=SV, OV=OV, flag=flagCollostructions, index=indexCollostructions), roleCorrelation=roleCorrelation, position=position, index=index, flag=flag, wordOrder=wordOrder)
}
...
SITUATION=function(speakerID){
	useCommonGround=world$useCommonGround; commonGroundStart=world$commonGroundStart; dahlS=world$dahlS; dahlA=world$dahlA; dahlO=world$dahlO; oddsNewExternal=world$oddsNewExternal; oddsNewInternal=world$oddsNewInternal; distinctions=world$distinctions; proportionIntrans=world$proportionIntrans; nEvents=world$nEvents; roleNoise=world$roleNoise; referenceNoise=world$referenceNoise; local=world$local; personTopicality=world$personTopicality; roleTopicality=world$roleTopicality; oddsSg=world$oddsSg
	...
	if(useCommonGround==T){
		...
		if(length(commonGround) < commonGroundStart){
			commonGround=c(commonGround, sample(speaker$nouns[!speaker$nouns$ID%in%commonGround,]$ID, commonGroundStart-length(commonGround))) #start with commonGround consisting of speech participants plus commonGroundStart element (necessary for sampling)
	}	} 
	#preparation
	...
	if(nIntrans!=0){
		...
		if(useCommonGround==T){
			intransExternals[intransExternals%in%c('3A','3I')]=paste(intransExternals[intransExternals%in%c('3A','3I')],sample(c('old','new'), length(intransExternals[intransExternals%in%c('3A','3I')]), replace=T, prob=c(1, oddsNewExternal)), sep='')
	...
	if(nTrans!=0){
		...
		if(useCommonGround==T){
			transExternals[transExternals%in%c('3A','3I')]=paste(transExternals[transExternals%in%c('3A','3I')],sample(c('old','new'), length(transExternals[transExternals%in%c('3A','3I')]), replace=T, prob=c(1, oddsNewExternal)), sep='')
			internals[internals%in%c('3A','3I')]=paste(internals[internals%in%c('3A','3I')],sample(c('old','new'), length(internals[internals%in%c('3A','3I')]), replace=T, prob=c(1, oddsNewExternal)), sep='')
	...
	roleTopic=sample(c('actor','undergoer','verb'),1, prob=roleTopicality)
	targetCandidates=vector()
	if(roleTopic=='actor'){
		targetCandidates=grep(personTopic, situation$personA)
		if(length(targetCandidates)==0){targetCandidates=grep(personTopic, situation$personU)}
	}
	if(roleTopic=='undergoer'){
		targetCandidates=grep(personTopic, situation$personU)
		if(length(targetCandidates)==0){targetCandidates=grep(personTopic, situation$personA)}
	}
	if(length(targetCandidates)==0 | roleTopic=='verb'){targetCandidates=1:nrow(situation)}
	target=ifelse(length(targetCandidates)==1, targetCandidates, sample(targetCandidates, 1))
	situation$target[target]=1
	situation$topic=roleTopic
	if(target%in%intrans & roleTopic=='undergoer'){situation$topic='actor'}
...
CANDIDATESCORE=function(lexicon, type='referringExpression'){
	candidateScoring=world$candidateScoring; distinctiveness=world$distinctiveness; activationImpact=world$activationImpact; semanticWeightImpact=world$semanticWeightImpact; frequency=world$frequency; frequencyImpact=world$frequencyImpact; collostructionImpact=world$collostructionImpact; recencyImpact=world$recencyImpact; economyImpact=world$economyImpact
	...
	if(candidateScoring=='activation'){
		if(frequency=='absolute' | !'argument'%in%names(lexicon)){	#absolute=relative for verbs
			lexicon$activation=lexicon$frequency/(lexicon$recency+1)	#decay of activation
			candidateScore=lexicon$activation
		}
		if(frequency=='relative' & 'argument'%in%names(lexicon)){
			if(type=='referringExpression'){lexicon$activation=lexicon$argument/(lexicon$recency+1)}
			if(type=='nounMarker'){lexicon$activation=lexicon$nounMarker/(lexicon$recency+1)}
			if(type=='verbMarker'){lexicon$activation=lexicon$verbMarker/(lexicon$recency+1)}
			candidateScore=lexicon$activation
	}	}
	...
	if(candidateScoring=='all'){
		if(frequency=='absolute' | !'argument'%in%names(lexicon)){
			lexicon$activation=lexicon$frequency/(lexicon$recency+1)+lexicon$collostruction 	#decay of activation, but collostruction frequency is stable
			candidateScore=lexicon$match + 
				semanticWeightImpact*min(lexicon$semanticWeight)/lexicon$semanticWeight + 	#normalize and rescale
				activationImpact*log(lexicon$activation + 2)/log(max(lexicon$activation) + 2) + 	# + 2 to prevent division by zero
				economyImpact*min(lexicon$productionEffort)/lexicon$productionEffort
		}
		if(frequency=='relative' & 'argument'%in%names(lexicon)){
			if(type=='referringExpression'){lexicon$activation=lexicon$argument/(lexicon$recency+1)+lexicon$collostruction}
			if(type=='nounMarker'){lexicon$activation=lexicon$nounMarker/(lexicon$recency+1)+lexicon$collostruction}
			if(type=='verbMarker'){lexicon$activation=lexicon$verbMarker/(lexicon$recency+1)+lexicon$collostruction}
			candidateScore=lexicon$match + 
				semanticWeightImpact*min(lexicon$semanticWeight)/lexicon$semanticWeight + 
				activationImpact*log(lexicon$activation + 2)/log(max(lexicon$activation) + 2) + 	# + 2 to prevent division by zero
				economyImpact*min(lexicon$productionEffort)/lexicon$productionEffort
	}	}	
	if(candidateScoring=='all2'){
		if(frequency=='absolute' | !'argument'%in%names(lexicon)){
			candidateScore=lexicon$match + 
				semanticWeightImpact*min(lexicon$semanticWeight)/lexicon$semanticWeight + 	#normalize and rescale
				frequencyImpact*log(lexicon$frequency + 2)/log(max(lexicon$frequency) + 2) + 		
				collostructionImpact*log(lexicon$collostruction + 2)/log(max(lexicon$collostruction) + 2) + 
				recencyImpact*min(lexicon$recency + 1)/(lexicon$recency + 1)  + 	
				economyImpact*min(lexicon$productionEffort)/lexicon$productionEffort
		}
		if(frequency=='relative' & 'argument'%in%names(lexicon)){
			if(type=='referringExpression'){
				candidateScore=lexicon$match + 
					semanticWeightImpact*min(lexicon$semanticWeight)/lexicon$semanticWeight + 
					frequencyImpact*log(lexicon$argument + 2)/log(max(lexicon$argument) + 2) + 
					collostructionImpact*log(lexicon$collostruction + 2)/log(max(lexicon$collostruction) + 2) + 
					recencyImpact*min(lexicon$recency + 1)/(lexicon$recency + 1)  + 
					economyImpact*min(lexicon$productionEffort)/lexicon$productionEffort
			}
			if(type=='nounMarker'){
				candidateScore=lexicon$match + 
					semanticWeightImpact*min(lexicon$semanticWeight)/lexicon$semanticWeight + 
					frequencyImpact*log(lexicon$nounMarker + 2)/log(max(lexicon$nounMarker) + 2) + 
					collostructionImpact*log(lexicon$collostruction + 2)/log(max(lexicon$collostruction) + 2) + 
					recencyImpact*min(lexicon$recency + 1)/(lexicon$recency + 1)  + 	
					economyImpact*min(lexicon$productionEffort)/lexicon$productionEffort
			}	
			if(type=='verbMarker'){
				candidateScore=lexicon$match + 
					semanticWeightImpact*min(lexicon$semanticWeight)/lexicon$semanticWeight + 
					frequencyImpact*log(lexicon$verbMarker + 2)/log(max(lexicon$verbMarker) + 2) + 
					collostructionImpact*log(lexicon$collostruction + 2)/log(max(lexicon$collostruction) + 2) + 
					recencyImpact*min(lexicon$recency + 1)/(lexicon$recency + 1)  + 
					economyImpact*min(lexicon$productionEffort)/lexicon$productionEffort
	}	}	}	
...
AGENTFIRSTPROD=function(proposition){
	if(proposition$verb$type=='twoPlace'){
		...
		external=proposition$verb[,grep('^Ext\\d',names(proposition$verb))]
		internal=proposition$verb[,grep('^Int\\d',names(proposition$verb))]
		if(ACTOR(external, internal)==1){actor='external'; undergoer='internal'} else {actor='internal'; undergoer='external'}
...
TOPICFIRSTPROD=function(speakerID, proposition){
	topicCopy=world$topicCopy; distinctiveness=world$distinctiveness; frequency=world$frequency
	speaker=population[[speakerID]]
	target=proposition$target
	if(proposition$verb$type=='onePlace'){
		if(proposition$verb$topic==1){proposition=proposition[c('verb','external')]}
		if(proposition$external$topic==1){proposition=proposition[c('external','verb')]}
		proposition$target=target
	}
	if(proposition$verb$type=='twoPlace'){
	...
	if(topicCopy==T & proposition$verb$topic!=1){
		markers=speaker$nouns[speaker$nouns$person==person,]	
		markers=markers[sample(nrow(markers)),]
		if(proposition$verb$type=='onePlace'){
		...
		if(proposition$verb$type=='twoPlace'){
			if(proposition$external$topic==1 & !'extMarkerID'%in%names(proposition$verb) | 
				proposition$internal$topic==1 & !'intMarkerID'%in%names(proposition$verb)){
				markers$externalMatch=VMATCH(proposition$external[,grep('^D\\d',names(proposition$external))], markers[,grep('^D\\d',names(markers))])
				markers$internalMatch=VMATCH(proposition$internal[,grep('^D\\d',names(proposition$internal))], markers[,grep('^D\\d',names(markers))])
				markers$collostruction=0
				if(proposition$external$topic==1){
					collostructions=speaker$collostructions$index[speaker$collostructions$index$N==proposition$external$ID,]
					markers[markers$ID%in%collostructions$marker,]$collostruction=collostructions[na.omit(match(markers$ID, collostructions$marker)),]$frequency
					names(markers)[grep('externalMatch', names(markers))]='match'
					names(markers)[grep('internalMatch', names(markers))]='distractorMatch'
				}
				if(proposition$internal$topic==1){
					collostructions=speaker$collostructions$index[speaker$collostructions$index$N==proposition$internal$ID,]
					markers[markers$ID%in%collostructions$marker,]$collostruction=collostructions[na.omit(match(markers$ID, collostructions$marker)),]$frequency
					names(markers)[grep('internalMatch', names(markers))]='match'
					names(markers)[grep('externalMatch', names(markers))]='distractorMatch'
				}
				markers=markers[order(CANDIDATESCORE(markers, type='verbMarker'), decreasing=T),]
				markerID=0
				for (i in 1:nrow(markers)){
				...
				if(proposition$external$topic==1){markerTarget=proposition$external$ID} else {markerTarget=proposition$internal$ID}
			}
			if(proposition$internal$topic==1 & !'intMarkerID'%in%names(proposition$verb)){
...
CHECKSUCCESS=function(speakerID, proposition, situation){
	solutionMethod=world$solutionMethod; distinctiveness=world$distinctiveness; agentFirst=world$agentFirst; frequency=world$frequency; referenceThreshold=world$referenceThreshold; generalization=world$generalization; wordOrder=world$wordOrder
	speaker=population[[speakerID]]
	...
	if(verb$type=='twoPlace'){
		...
		if(agentFirst==T & grep(actor,names(proposition)) < grep(undergoer,names(proposition))){elaborate='unnecessary'}		#if agentFirst can be used, elaboration is unnecessary
		#first try if indexes are informative 
		if(elaborate=='necessary'){	
			if('extMarker'%in%names(verb) & nrow(speaker$nouns[speaker$nouns$person==external$person & speaker$nouns$productionEffort<world$referenceThreshold,])>1){
				indexes=speaker$nouns[speaker$nouns$person==external$person & speaker$nouns$productionEffort<world$referenceThreshold,]
				indexes$typing=VMATCH(verb[,grep('^Ext\\d',names(verb))], indexes[, grep('^D\\d',names(indexes))])
				index=indexes[MAX(indexes$typing, forceChoice=T),]
				proposition$verb$extMarkerID=index$ID
				proposition$verb$extMarker=index$form
				proposition$verb$extMarkerFrequency=ifelse(frequency=='absolute',index$frequency, index$verbMarker)
				extMarkerCollostructionFrequency=speaker$collostructions$index[intersect(grep(paste('^', index$ID, '$', sep=''), speaker$collostructions$index$marker),grep(paste('^', proposition$external$ID, '$', sep=''), speaker$collostructions$index$N)),]$frequency
				...
				if(indexes[indexes$ID==proposition$verb$extMarkerID,]$typing >= (max(indexes[indexes$ID!=proposition$verb$extMarkerID,]$typing) + distinctiveness)) {elaborate='unnecessary'}
			}
			if('intMarker'%in%names(verb) & nrow(speaker$nouns[speaker$nouns$person==internal$person & speaker$nouns$productionEffort<world$referenceThreshold,])>1){
				indexes=speaker$nouns[speaker$nouns$person==internal$person & speaker$nouns$productionEffort<world$referenceThreshold,]
				indexes$typing=VMATCH(verb[,grep('^Int\\d',names(verb))], indexes[, grep('^D\\d',names(indexes))])
				index=indexes[MAX(indexes$typing, forceChoice=T),]
				proposition$verb$intMarkerID=index$ID
				proposition$verb$intMarker=index$form
				proposition$verb$intMarkerFrequency=ifelse(frequency=='absolute',index$frequency, index$verbMarker)
				intMarkerCollostructionFrequency=speaker$collostructions$index[intersect(grep(paste('^', index$ID, '$', sep=''), speaker$collostructions$index$marker),grep(paste('^', proposition$internal$ID, '$', sep=''), speaker$collostructions$index$N)),]$frequency
				...
				if(indexes[indexes$ID==proposition$verb$intMarkerID,]$typing >= (max(indexes[indexes$ID!=proposition$verb$intMarkerID,]$typing) + distinctiveness)) {elaborate='unnecessary'}
		}	}
...
#					internal$number=number
		...
		if(generalization==T & wordOrder==T){
			#any particular order
			if(TRUE %in% c((sum(speaker$wordOrder$success)-speaker$wordOrder$success) < (sum(speaker$wordOrder$success)/log(sum(speaker$wordOrder$success))))){elaborate='unnecessary'}
			#or combination of S first and V-O
			if(TRUE %in% c(sum(speaker$wordOrder[grep('^S', speaker$wordOrder$order, invert=T), ]$success) < (sum(speaker$wordOrder$success)/log(sum(speaker$wordOrder$success)))) & (c(sum(speaker$wordOrder[grep('OV', speaker$wordOrder$order, invert=T), ]$success) < (sum(speaker$wordOrder$success)/log(sum(speaker$wordOrder$success)))) | c(sum(speaker$wordOrder[grep('VO', speaker$wordOrder$order, invert=T), ]$success) < (sum(speaker$wordOrder$success)/log(sum(speaker$wordOrder$success)))))){elaborate='unnecessary'}
		}
...
GENERALIZE=function(speakerID, proposition){
	distinctiveness=world$distinctiveness; frequency=world$frequency; suffixThreshold=world$suffixThreshold; distinctions=world$distinctions
	speaker=population[[speakerID]]
	#check generalizations(using Yang's tolerance principle)
	yangIndex=TRUE %in% c(speaker$index$no < ((speaker$index$yes + speaker$index$no)/log(speaker$index$yes + speaker$index$no)))
	yangPerson=TRUE %in% c(speaker$flag$person$no < ((speaker$flag$person$yes + speaker$flag$person$no)/log(speaker$flag$person$yes + speaker$flag$person$no)))
	yangGrammaticalRole=TRUE %in% c(speaker$flag$external$no < ((speaker$flag$external$yes + speaker$flag$external$no)/log(speaker$flag$external$yes + speaker$flag$external$no))) | TRUE %in% c(speaker$flag$internal$no < ((speaker$flag$internal$yes + speaker$flag$internal$no)/log(speaker$flag$internal$yes + speaker$flag$internal$no)))
	yangSemanticRole=TRUE %in% c(speaker$flag$actor$no < ((speaker$flag$actor$yes + speaker$flag$actor$no)/log(speaker$flag$actor$yes + speaker$flag$actor$no))) | TRUE %in% c(speaker$flag$undergoer$no < ((speaker$flag$undergoer$yes + speaker$flag$undergoer$no)/log(speaker$flag$undergoer$yes + speaker$flag$undergoer$no)))
	...
	if(sum(speaker$wordOrder$success)>8){	#n exceptions should minimally be 4(=8/ln(8)) for Yang
		yangWordOrder=TRUE %in% 
			...
			#S first
			c(sum(speaker$wordOrder[grep('^S', speaker$wordOrder$order, invert=T), ]$success) < (sum(speaker$wordOrder$success)/log(sum(speaker$wordOrder$success)))) |
			#OV
			c(sum(speaker$wordOrder[grep('OV', speaker$wordOrder$order, invert=T), ]$success) < (sum(speaker$wordOrder$success)/log(sum(speaker$wordOrder$success)))) |
			#VO
			c(sum(speaker$wordOrder[grep('VO', speaker$wordOrder$order, invert=T), ]$success) < (sum(speaker$wordOrder$success)/log(sum(speaker$wordOrder$success)))) 
		if(yangWordOrder==T){
			##first general generalizations
			#S first
			if(TRUE %in% c(sum(speaker$wordOrder[grep('^S', speaker$wordOrder$order, invert=T), ]$success) < (sum(speaker$wordOrder$success)/log(sum(speaker$wordOrder$success))))){
				proposition=proposition[c(grep('external', names(proposition)), grep('external', names(proposition), invert=T))]				
			}
			if(proposition$verb$type=='twoPlace'){
				#OV
				if(TRUE %in% c(sum(speaker$wordOrder[grep('OV', speaker$wordOrder$order, invert=T), ]$success) < (sum(speaker$wordOrder$success)/log(sum(speaker$wordOrder$success))))){
					if(grep('external', names(proposition)) < grep('verb', names(proposition))){proposition=proposition[c('external', 'internal', 'verb', 'target')]}
					if(grep('external', names(proposition)) > grep('verb', names(proposition))){proposition=proposition[c('internal', 'verb', 'external', 'target')]}				
				}
				#VO
				if(TRUE %in% c(sum(speaker$wordOrder[grep('VO', speaker$wordOrder$order, invert=T), ]$success) < (sum(speaker$wordOrder$success)/log(sum(speaker$wordOrder$success))))){
					if(grep('external', names(proposition)) < grep('verb', names(proposition))){proposition=proposition[c('external', 'verb', 'internal', 'target')]}
					if(grep('external', names(proposition)) > grep('verb', names(proposition))){proposition=proposition[c('verb', 'internal', 'external', 'target')]}				
				}
				...
				if(TRUE %in% c((sum(speaker$wordOrder$success)-speaker$wordOrder$success) < (sum(speaker$wordOrder$success)/log(sum(speaker$wordOrder$success))))){
					...
					order=unlist(strsplit(order, '')); order=gsub('S', 'external', order); order=gsub('O', 'internal', order); order=gsub('V', 'verb', order)
					proposition=proposition[c(order, 'target')]
	}	}	}	}
	#... then morphological marking (since solutionMethod sometimes dependent on word order)
	if(TRUE %in% c(yangIndex, yangPerson, yangGrammaticalRole, yangSemanticRole)){
		if(proposition$verb$type=='onePlace'){
			verbRole='external'
	...
	if(yangIndex==T){
		#first argument
		index=F
		data=speaker$index[speaker$index$person==firstArgument$person & speaker$index$role%in%c(firstArgument$verbRole, firstArgument$semRole),]
		if(TRUE %in% c(data$no < (data$yes + data$no)/log(data$yes + data$no))){index=T}		
		if(firstArgument$verbRole=='external' & 'extMarker'%in%names(proposition$verb) | firstArgument$verbRole=='internal' & 'inMarker'%in%names(proposition$verb)){index=F}
		if(index==T){	
		...
		if(proposition$verb$type=='twoPlace'){
			index=F
			data=speaker$index[speaker$index$person==secondArgument$person & speaker$index$role%in%c(secondArgument$verbRole, secondArgument$semRole),]
			if(TRUE %in% c(data$no < (data$yes + data$no)/log(data$yes + data$no))){index=T}		
			if(secondArgument$verbRole=='external' & 'extMarker'%in%names(proposition$verb)){index=F}
			if(secondArgument$verbRole=='internal' & 'inMarker'%in%names(proposition$verb)){index=F}
			if(index==T){	
	...
	#flag
	if(yangPerson==T | yangGrammaticalRole==T | yangSemanticRole==T){
		#first argument
		data=speaker$flag$person[speaker$flag$person$role%in%c(firstArgument$verbRole, firstArgument$semRole) & speaker$flag$person$person==firstArgument$person,]
		flag=TRUE %in% c(data$no < (data$yes + data$no)/log(data$yes + data$no))	#can flagging rule be derived at general level?
		#or for specific dimensions of verb role (for third person only)?
		if(flag==FALSE & firstArgument$person==3){	
			values=unlist(rep(firstArgument$props, distinctions)); values[is.na(values)]=-1
			values=values[1:length(speaker$flag[[firstArgument$verbRole]]$value)]		#necessary for economically stored resurrected agents;)
			data=speaker$flag[[firstArgument$verbRole]][speaker$flag[[firstArgument$verbRole]]$value==values,]
			flag=TRUE %in% c(data$no < (data$yes + data$no)/log(data$yes + data$no))	#can flagging rule be derived at general level?
		}
		#or for specific dimensions of semantic role?
		if(flag==FALSE){	
			values=unlist(rep(firstArgument$props, distinctions)); values[is.na(values)]=-1
			values=values[1:length(speaker$flag[[firstArgument$semRole]]$value)]		#necessary for economically stored resurrected agents;)
			data=speaker$flag[[firstArgument$semRole]][speaker$flag[[firstArgument$semRole]]$value==values,]
			flag=TRUE %in% c(data$no < (data$yes + data$no)/log(data$yes + data$no))	#can flagging rule be derived at general level?
		}
		if(flag==T){
...
#				proposition[[firstArgument$verbRole]]$number=number
		...
		#second argument
		if(proposition$verb$type=='twoPlace'){
			data=speaker$flag$person[speaker$flag$person$role%in%c(secondArgument$verbRole, secondArgument$semRole) & speaker$flag$person$person==secondArgument$person,]
			flag=TRUE %in% c(data$no < (data$yes + data$no)/log(data$yes + data$no))	#can flagging rule be derived at general level?
			if(flag==FALSE & secondArgument$person==3){	#or for specific dimensions of actor role (for third person only)?
				values=unlist(rep(secondArgument$props, distinctions)); values[is.na(values)]=-1
				values=values[1:length(speaker$flag[[secondArgument$verbRole]]$value)]		#necessary for economically stored resurrected agents;)
				data=speaker$flag[[secondArgument$verbRole]][speaker$flag[[secondArgument$verbRole]]$value==values,]
				flag=TRUE %in% c(data$no < (data$yes + data$no)/log(data$yes + data$no))	#can flagging rule be derived at general level?
			}
			if(flag==FALSE){	#or for specific dimensions of verb role?
				values=unlist(rep(secondArgument$props, distinctions)); values[is.na(values)]=-1
				values=values[1:length(speaker$flag[[secondArgument$semRole]]$value)]		#necessary for economically stored resurrected agents;)
				data=speaker$flag[[secondArgument$semRole]][speaker$flag[[secondArgument$semRole]]$value==values,]
				flag=TRUE %in% c(data$no < (data$yes + data$no)/log(data$yes + data$no))	#can flagging rule be derived at general level?
			}
			if(flag==T){
...
FIRSTINFIRSTOUT=function(speakerID, proposition){
	...
	for (i in 1:(length(activations)-1)){	#skip target
		if(frequency=='absolute'){activations[i]=proposition[[i]]$frequency/(proposition[[i]]$recency+1)+proposition[[i]]$collostruction}
		if(frequency=='relative'){
			#nouns
			if('argument' %in% names(proposition[[i]])){activations[i]=proposition[[i]]$argument/(proposition[[i]]$recency+1)+proposition[[i]]$collostruction}
			#verbs
			if(!'argument' %in% names(proposition[[i]])){activations[i]=proposition[[i]]$frequency/(proposition[[i]]$recency+1)+proposition[[i]]$collostruction}
	}	}
...
PREPARE=function(speakerID, proposition, situation){
	generalization=world$generalization; refCheck=world$refCheck; typeCast=world$typeCast; agentFirst=world$agentFirst; topicFirst=world$topicFirst; orderAgentFirstTopicFirst=world$orderAgentFirstTopicFirst; checkSuccess=world$checkSuccess; firstInFirstOut=world$firstInFirstOut; erosion=world$erosion
	prep=proposition
	...
	if(agentFirst==T & topicFirst==F){prep=AGENTFIRSTPROD(prep)}
	if(topicFirst==T  & agentFirst==F){prep=TOPICFIRSTPROD(speakerID, prep)}
	if(agentFirst==T & topicFirst==T){
		if(orderAgentFirstTopicFirst=='random'){orderAgentFirstTopicFirst=sample(c('AT', 'TA'), 1)}
		if(orderAgentFirstTopicFirst=='AT'){prep=TOPICFIRSTPROD(speakerID, prep); prep=AGENTFIRSTPROD(prep)} #priority means final say
		if(orderAgentFirstTopicFirst=='TA'){prep=AGENTFIRSTPROD(prep); prep=TOPICFIRSTPROD(speakerID, prep)}
	}
...
PRODUCE=function(speakerID, prep){
	suffixThreshold=world$suffixThreshold; fusionThreshold=world$fusionThreshold; fusionThreshold2=world$talkAge*world$deathAge
	fusionThreshold=fusionThreshold*max(population[[speakerID]]$age, fusionThreshold2, na.rm=T)
	...
	if('internal'%in%names(prep)){
		...
		if('marker'%in%names(prep$internal)){	
			if(PRODEFFORT(prep$internal$marker) <= suffixThreshold | prep$internal$markerCollostruction >= fusionThreshold){marker=prep$internal$marker; internal=paste(internal, marker, sep='')}
			if(PRODEFFORT(prep$internal$marker) > suffixThreshold & prep$internal$markerCollostruction < fusionThreshold){marker=prep$internal$marker; internal=paste(internal, marker, sep=' ')}
		}
		if('verb'%in%names(prep)){
	...
	if('external'%in%names(prep)){
		...
		if('marker'%in%names(prep$external)){	
			if(PRODEFFORT(prep$external$marker) <= suffixThreshold | prep$external$markerCollostruction >= fusionThreshold){marker=prep$external$marker; external=paste(external, marker, sep='')}
			if(PRODEFFORT(prep$external$marker) > suffixThreshold & prep$external$markerCollostruction < fusionThreshold){marker=prep$external$marker; external=paste(external, marker, sep=' ')}
		}
		if('verb'%in%names(prep)){
...
DECOMPOSE=function(hearerID, form){
	...
	if(length(suffixes)!=0){
		suffixes[nchar(suffixes)>1]=gsub('.$','.?$',suffixes[nchar(suffixes)>1])
		suffixes[nchar(suffixes)==1]=gsub('.$','.$',suffixes[nchar(suffixes)==1])
...
ANALYZE=function(hearerID, utterance, situation){
	...
	analysis$verbID=0; analysis$verb=''; analysis$verbType=''; analysis$verbMatch=0; analysis$actionMatch=0; analysis$verbScore=0; analysis$verbEvidence=0
	analysis$nounID=0; analysis$noun=''; analysis$nounPerson=0; analysis$nounMatch=0
	analysis$nounMarkerID=0; analysis$nounMarker=''; analysis$nounMarkerPerson=0
	analysis$verbMarkerID=0; analysis$verbMarker=''; analysis$verbMarkerPerson=0
	for (i in 1:nrow(analysis)){
		if(analysis$role[i]=='suffix'){
			candidates=hearer$nouns
			candidates$collostruction=0
			candidates$match=FMATCH(analysis$stem[i], candidates)
			entry=candidates[order(CANDIDATESCORE(candidates, type='nounMarker'), decreasing=T)[1],]
			analysis$nounMarkerID[i]=entry$ID
			analysis$nounMarkerPerson[i]=entry$person
			analysis$nounMarker[i]=entry$form
			entry=candidates[order(CANDIDATESCORE(candidates, type='verbMarker'), decreasing=T)[1],]
			analysis$verbMarkerID[i]=entry$ID
			analysis$verbMarkerPerson[i]=entry$person
			analysis$verbMarker[i]=entry$form
			entry=candidates[order(CANDIDATESCORE(candidates), decreasing=T)[1],]	#suffix could be incorporated noun
		...
		if(analysis$role[i]=='?'){
			...
			candidates$collostruction=0
			candidates$match=FMATCH(analysis$stem[i], candidates)
			entry=candidates[order(CANDIDATESCORE(candidates), decreasing=T)[1],]
			...
			entry=candidates[order(CANDIDATESCORE(candidates, type='verbMarker'), decreasing=T)[1],]	#or a verb adposition
			analysis$verbMarkerID[i]=entry$ID
			analysis$verbMarkerPerson[i]=entry$person
			analysis$verbMarker[i]=entry$form
			entry=candidates[order(CANDIDATESCORE(candidates, type='nounMarker'), decreasing=T)[1],]		#or a noun adposition
			analysis$nounMarkerID[i]=entry$ID
			analysis$nounMarkerPerson[i]=entry$person
			analysis$nounMarker[i]=entry$form
	}	}
	...
	if(nrow(analysis)==1){	#if there's only 1 word in the utterance
		analysis$role=ifelse(analysis$verbScore > analysis$nounMatch, 'verb', '?')	#default interpretation is noun (cf. H & K)
	} 
	if(nrow(analysis) > 1){
		#first identify verb
		...
		for (i in candidates){
			...
			argumentScores=tapply(analysis[arguments, ]$nounMatch, analysis[arguments,]$word, max)
			analysis$verbEvidence[i]=prod(analysis$verbScore[i], argumentScores)
		}
		verbIndex=MAX(analysis$verbEvidence)
		if(length(verbIndex) > 1){verbIndex=verbIndex[MIN(analysis[verbIndex,]$nounMatch)]}
		...
		if(length(wrong)!=0){
			for (i in wrong){
				if(analysis$role[i-1]=='?'){analysis$nounMatch[i-1]=0}
				if(analysis$role[i-1]=='nounSuffix'){analysis$nounMatch[i-2]=0}
		}	}
		#choose morphological analysis
		for (word in unique(analysis[grep('\\?',analysis$role),]$word)){
			option=MAX(analysis[intersect(grep('\\?', analysis$role), grep(word, analysis$word)),]$nounMatch, forceChoice=T)
...
GROUP=function(hearerID, analysis){
	distinctiveness=world$distinctiveness; topicCopy=world$topicCopy; topicFirst=world$topicFirst
	hearer=population[[hearerID]]
	...
	verbAdposition=0
	if(verb!=0){
		verbAdposition=verb + 1     #1 verbAdposition (for topic cross reference)... 
	...
	for (i in 1:nrow(analysis)){    
		...
		if(i==verbAdposition & analysis$role[1]=='?'){words[[i]]=c('verbAdposition','?')}    #only verb adposition if preposed topic
		if(i==verbAdposition & analysis$role[1]!='?'){words[[i]]='?'}    #only verb adposition if preposed topic
...
WORDORDER=function(hearerID, analysis){
	...
	verb=grep(grep('^verb$', analysis$role), positions)
	if(sum(hearer$wordOrder$success)>8){	#n exceptions should minimally be 4(=8/ln(8)) for Yang
		if(analysis[analysis$role=='verb',]$verbType=='twoPlace'){
			...
			if(TRUE %in% c((sum(hearer$wordOrder$success)-hearer$wordOrder$success) < (sum(hearer$wordOrder$success)/log(sum(hearer$wordOrder$success))))){
				...
				external=grep('S', order)-grep('V', order)
				if( (verb+external)>0 & (verb+external)<=length(positions) ){
					if('external'%in%analysis$role){analysis[analysis$role=='external',]$role='?'}	#overwrite more general tendencies
					analysis[positions[verb+external],]$role='external'
				}
				internal=grep('O', order)-grep('V', order)
				if( (verb+internal)>0 & (verb+internal)<=length(positions) ){
					if('internal'%in%analysis$role){analysis[analysis$role=='internal',]$role='?'}
					analysis[positions[verb+internal],]$role='internal'
			}	}
			##no particular order
			if(!TRUE %in% c((sum(hearer$wordOrder$success)-hearer$wordOrder$success) < (sum(hearer$wordOrder$success)/log(sum(hearer$wordOrder$success))))){
				#S first
				if(TRUE %in% c(sum(hearer$wordOrder[grep('^S', hearer$wordOrder$order, invert=T), ]$success) < (sum(hearer$wordOrder$success)/log(sum(hearer$wordOrder$success))))){
					if(analysis[1,]$role=='?'){analysis[1,]$role='external'}
				}
				#OV
				if(TRUE %in% c(sum(hearer$wordOrder[grep('OV', hearer$wordOrder$order, invert=T), ]$success) < (sum(hearer$wordOrder$success)/log(sum(hearer$wordOrder$success))))){
					if(verb!=1){analysis[positions[verb-1],]$role='internal'}
				}	
				#VO
				if(TRUE %in% c(sum(hearer$wordOrder[grep('VO', hearer$wordOrder$order, invert=T), ]$success) < (sum(hearer$wordOrder$success)/log(sum(hearer$wordOrder$success))))){
					if((verb+1)<=length(positions)){analysis[positions[verb+1],]$role='internal'}
	}	}	}	}
analysis			
}
...
VERBMORPHOLOGY=function(hearerID, analysis){
	refCheck=world$refCheck; referenceThreshold=world$referenceThreshold; distinctiveness=world$distinctiveness; collostructionImpact=world$collostructionImpact
	hearer=population[[hearerID]]
	nouns=hearer$nouns
	verbs=hearer$verbs
	##resolution/indexing
	if('verbSuffix'%in%analysis$role | 'verbAdposition'%in%analysis$role){analysis$verbMarkerTarget='?'}
	if('verbAdposition'%in%analysis$role){		
		if(analysis[1,]$role=='?' | analysis[1,]$role=='external' | analysis[1,]$role=='internal'){	#only verb-adposition analysis with topicalized argument
			analysis[grep('verbAdposition',analysis$role), ]$verbMarkerTarget=1
		} else {
	...
	if('verbSuffix'%in%analysis$role){	
			...
			if(length(targets)==1 & length(verbSuffixes)==1){
				...
				if(refCheck==T){
					if(analysis[verbSuffixes,]$verbMarkerPerson!=analysis[targets,]$nounPerson){
						if(analysis[verbSuffixes,]$form<referenceThreshold & analysis[targets,]$nounPerson==3){
							analysis[verbSuffixes,]$verbMarkerTarget=targets
				}	}	}
				#person match (overrules recruitment solution)
				if(analysis[verbSuffixes,]$verbMarkerPerson==analysis[targets,]$nounPerson){
					analysis[verbSuffixes,]$verbMarkerTarget=targets
		}	}	}	
		###transitives
			...
			if(length(verbSuffixes)==1){
				...
				if(length(grep(analysis[v,]$verbMarkerPerson, analysis[targets,]$nounPerson))>1){
					...
					if(nrow(data)!=0){
						...
						if(role%in%proto$role){analysis[v,]$verbMarkerTarget=grep(role, proto$role)}
					}	
					#use semantics and usage history if suffix was result of choice (and no generalization was made)
					if(nrow(data)==0){
						if(nrow(nouns[nouns$person==analysis[v,]$verbMarkerPerson & nouns$verbMarker>0,])>1){
							verbSuffixProfile=nouns[nouns$ID==analysis[v,]$verbMarkerID,grep('^D\\d',names(nouns))]
							match=VMATCH(verbSuffixProfile, nouns[match(analysis[targets,]$nounID, nouns$ID),grep('^D\\d',names(nouns))])
							person=rep(-1, length(targets))
							person[grep(analysis[v,]$verbMarkerPerson, analysis[targets,]$nounPerson)]=0
							collostructions=hearer$collostructions$index[hearer$collostructions$index$marker==analysis[v,]$verbMarkerID,]
							collostruction=rep(0, length(targets))
							if(nrow(collostructions)!=0){collostruction[analysis[targets,]$nounID%in%collostructions$n]=collostructions[na.omit(match(analysis[targets,]$nounID, collostructions$N)),]$frequency/(max(collostructions$frequency) + 1)}
							suffixLink=-1
							suffixLink=match + person + collostructionImpact*collostruction
							analysis[v,]$verbMarkerTarget=targets[MAX(suffixLink, forceChoice=T)]
				}	}	}
				#no match, recruitment?
				if(refCheck==T){
					if(!analysis[v,]$verbMarkerPerson %in% analysis[targets,]$nounPerson & length(targets)!=0){
						if(PRODEFFORT(analysis[v,]$form)<referenceThreshold){
							#single target
							...
							if(length(grep(3, analysis[targets,]$nounPerson))>1){
								...
								data=hearer$index[hearer$index$person==person,]
								data=data[data$no < (data$yes + data$no)/log(data$yes + data$no),]
								#if generalization is made, use role
								if(nrow(data)!=0){
									role=data$role[MAX((data$yes + data$no)/log(data$yes + data$no), forceChoice=T)]
									if(role=='actor'){role=ifelse(ACTOR(verb[,grep('^Ext\\d',names(verb))], verb[,grep('^Int\\d',names(verb))])==1, 'external', 'internal')}
									if(role=='undergoer'){role=ifelse(ACTOR(verb[,grep('^Ext\\d',names(verb))], verb[,grep('^Int\\d',names(verb))])==1, 'internal', 'external')}
				...
				for(v in verbSuffixes){
					...
					if(length(grep(analysis[v,]$verbMarkerPerson, analysis[targets,]$nounPerson))>1){
						if(grep(v, verbSuffixes)==1){
							if('internal'%in%proto$role){analysis[v,]$verbMarkerTarget=grep('internal', proto$role)}
						}
						if(grep(v, verbSuffixes)==2){
							if('external'%in%proto$role){analysis[v,]$verbMarkerTarget=grep('external', proto$role)}
					}	}
		...
		##role marking/reanalysis
		targets=grep('\\?', analysis$verbMarkerTarget, invert=T)
		verbSuffixes=grep('verbSuffix', analysis$role)
		if(verb$type=='onePlace'){
		...
		for(v in verbSuffixes){
			if(analysis[v,]$verbMarkerTarget!='?'){
				person=analysis[v,]$verbMarkerPerson
				data=hearer$index[hearer$index$person==person,]
				data=data[data$no < (data$yes + data$no)/log(data$yes + data$no),]
				if(nrow(data)!=0){
					...
					if(role=='actor'){role=ifelse(ACTOR(verb[,grep('^Ext\\d',names(verb))], verb[,grep('^Int\\d',names(verb))])==1, 'external', 'internal')}
					if(role=='undergoer'){role=ifelse(ACTOR(verb[,grep('^Ext\\d',names(verb))], verb[,grep('^Int\\d',names(verb))])==1, 'internal', 'external')}
					if(analysis[analysis[v,]$verbMarkerTarget,]$role=='?'){analysis[analysis[v,]$verbMarkerTarget,]$role=role}
		}	}	}
		#defaults
		if(verb$type=='twoPlace'){
			if(length(targets)==0 & length(grep('\\?', analysis$role))<2 & length(verbSuffixes)==1){analysis[verbSuffixes,]$role='?'}	#if no targets, reanalyse as deictic
			if(length(targets)==0 & length(verbSuffixes)==2){analysis[verbSuffixes,]$role=c('internal', 'external')}	#if no targets, reanalyse as deictic; inner verb suffix has internal role
			if(length(targets)==2 & length(verbSuffixes)==2){
				if(!'?'%in%analysis[verbSuffixes,]$verbMarkerTarget){
					if(analysis[analysis[verbSuffixes[1],]$verbMarkerTarget,]$role=='?' & !'internal'%in%analysis$role){analysis[analysis[verbSuffixes[1],]$verbMarkerTarget,]$role='internal'}
					if(analysis[analysis[verbSuffixes[2],]$verbMarkerTarget,]$role=='?' & !'external'%in%analysis$role){analysis[analysis[verbSuffixes[2],]$verbMarkerTarget,]$role='external'}
	}	}	}	}
analysis
}

PROTOINTERPRETATION=function(hearerID, analysis){
	agentFirst=world$agentFirst
	if('?'%in%analysis$role){
		if(agentFirst==F){
		...
		if(agentFirst==T){
			...
			if(analysis[analysis$role=='verb',]$verbType=='onePlace'){
				if(!'external'%in%analysis$role){
					agentFirst=AGENTFIRSTINT(hearerID, analysis)
					if(!'external'%in%analysis$role & 'external'%in%agentFirst$role){analysis[grep('external', agentFirst$role),]$role='external'}
			}	}
			if(analysis[analysis$role=='verb',]$verbType=='twoPlace'){
				if(!'external'%in%analysis$role | !'internal'%in%analysis$role){
					agentFirst=AGENTFIRSTINT(hearerID, analysis)
					if(!'external'%in%analysis$role & 'external'%in%agentFirst$role){analysis[grep('external', agentFirst$role),]$role='external'}
					if(!'internal'%in%analysis$role & 'internal'%in%agentFirst$role){analysis[grep('internal', agentFirst$role),]$role='internal'}
			}	}
			#TYPEMATCH
			if(analysis[analysis$role=='verb',]$verbType=='onePlace'){
				if(!'external'%in%analysis$role){
...
INTERPRET.INT=function(hearerID, analysis, situation){	
	...
	if(!'verb'%in%analysis$role){analysis$role='external'}
	#result
#	if(!'?'%in%analysis$role){	##UITPROBEREN OF VOORWAARDE KAN VERWIJDERD##
		verb=''; internal=''
...
#			verb$number=analysis[analysis$role=='verb',]$verbNumber
			if('verbSuffix'%in%analysis$role){
				if(length(unique(analysis$verbMarkerTarget[analysis$verbMarkerTarget!='?']))==1){	#single target
					i=grep('verbSuffix', analysis$role)[1]	
					if('internal'%in%analysis[analysis[i,]$verbMarkerTarget,]$role){	
						...
						i=grep('verbSuffix', analysis$role)[2]	#if two suffixes with identical target, inner suffix is internal
					}
				...
				if(length(unique(analysis$verbMarkerTarget[analysis$verbMarkerTarget!='?']))!=1){	#different targets
			...
			if('verbAdposition'%in%analysis$role){	#NB verbAdpositions overrule verbSuffixes... (for this is what they are used for)
...
#				external$number=analysis[analysis$role=='external',]$nounNumber
		...
		if(nrow(situation) > 1){
			if(is.data.frame(verb)){situation$verbMatch=VMATCH(verb[,grep('^D\\d',names(verb))], situation[,grep('^V\\d',names(situation))])} else {
				situation$verbMatch=0}
			if(is.data.frame(external)){
				if(external$person==3){
					...
					if(sum(candidates==3, na.rm=T)==1){situation$externalMatch=ifelse(external$person==situation$personA, 1, 0)}
					if(sum(candidates==3, na.rm=T)==0){situation$externalMatch=0}
				}
				if(external$person!=3){situation$externalMatch=ifelse(external$person==situation$personA, 1, 0)}
			} else {situation$externalMatch=0}
			if(is.data.frame(internal)){	#implies that twoPlace verb has been identified (cf. previous block)
				if(ACTOR(verb[,grep('^Ext\\d',names(verb))], verb[,grep('^Int\\d',names(verb))])==1){
					if(internal$person==3){
						...
						if(sum(candidates==3, na.rm=T)==1){situation$internalMatch=ifelse(internal$person==situation$personU, 1, 0)}
						if(sum(candidates==3, na.rm=T)==0){situation$internalMatch=0}
					}
					if(internal$person!=3){situation$internalMatch=ifelse(internal$person==situation$personU, 1, 0)}
				}
				if(ACTOR(verb[,grep('^Ext\\d',names(verb))], verb[,grep('^Int\\d',names(verb))])==2){
					if(internal$person==3){
						...
						if(sum(candidates==3, na.rm=T)==1){situation$internalMatch=ifelse(internal$person==situation$personA, 1, 0)}
						if(sum(candidates==3, na.rm=T)==0){situation$internalMatch=0}
					}
					if(internal$person!=3){situation$internalMatch=ifelse(internal$person==situation$personA, 1, 0)}
					if(is.data.frame(external)){
						if(external$person==3){
							candidates=situation$personU
							...
							if(sum(candidates==3, na.rm=T)==1){situation$externalMatch=ifelse(external$person==situation$personU, 1, 0)}
							if(sum(candidates==3, na.rm=T)==0){situation$externalMatch=0}
						}
						if(external$person!=3){situation$externalMatch=ifelse(external$person==situation$personU, 1, 0)}
					} else {situation$externalMatch=0}
				}
			} else {situation$internalMatch=0}
			if (!is.data.frame(internal)){
				if(sum(situation$U1, na.rm=T)!=0){situation[grep('\\d', situation$U1),]$internalMatch=-1}	#internal was present in situation but not understood
			}
			situation$totalMatch=situation$verbMatch + situation$externalMatch + situation$internalMatch
		...
		if(!is.data.frame(verb)){interpretation=list(target=target, external=external)}
		if(is.data.frame(verb)){
			if(verb$type=='onePlace'){interpretation=list(target=target, verb=verb, external=external)}
			if(verb$type=='twoPlace'){interpretation=list(target=target, verb=verb, external=external, internal=internal)}
		}
...
INTERPRET=function(hearerID, utterance, situation){
	wordOrder=world$wordOrder; frequencyImpact=world$frequencyImpact; collostructionImpact=world$collostructionImpact
	hearer=population[[hearerID]]
	...
	for(j in 1:length(grouping)){
		...
		if(length(interpretation.int)!=0){interpretation[[length(interpretation) + 1]]=interpretation.int}
	}	
		...
		if(nEvents==1){
			...
			for(i in 1:length(interpretation)){
				...
				scores[i]=0
					...
					scores[i]=scores[i] + frequencyImpact*interpretation[[i]]$verb$extMarkerFrequency + collostructionImpact*interpretation[[i]]$verb$extMarkerCollostructionFrequency
				}
				if('intMarkerID'%in%names(verb)){
					scores[i]=scores[i] + frequencyImpact*interpretation[[i]]$verb$intMarkerFrequency + collostructionImpact*interpretation[[i]]$verb$intMarkerCollostructionFrequency
				}
				if(is.data.frame(interpretation[[i]]$external)){
					...
					scores[i]=scores[i] + frequencyImpact*interpretation[[i]]$external$argument + collostructionImpact*interpretation[[i]]$external$collostruction
					if('marker'%in%names(external)){scores[i]=scores[i] + frequencyImpact*external$markerFrequency}
				}
				if(is.data.frame(interpretation[[i]]$internal)){
					...
					scores[i]=scores[i] + frequencyImpact*interpretation[[i]]$internal$argument + collostructionImpact*interpretation[[i]]$internal$collostruction
					if('marker'%in%names(internal)){scores[i]=scores[i] + frequencyImpact*internal$markerFrequency}
	}	}	}	}
...
FREQUPDATE=function(agentID, meaning, success){
	...
	if(is.list(meaning)){
		...
		if(is.data.frame(meaning$verb)){
			...
			if(success==1){
				if(is.data.frame(meaning$external)){
					verbRole=grep('external',agent$index$role)
					person=grep(meaning$external$person, agent$index$person)
					semRole='actor'
					...
					semRole2=grep(semRole,agent$index$role)
					if('extMarker'%in%names(meaning$verb)){
						agent$index[intersect(person, verbRole),]$yes=agent$index[intersect(person, verbRole),]$yes + 1
						agent$index[intersect(person, semRole2),]$yes=agent$index[intersect(person, semRole2),]$yes + 1
					}
					if(!'extMarker'%in%names(meaning$verb)){
						agent$index[intersect(person, verbRole),]$no=agent$index[intersect(person, verbRole),]$no + 1
						agent$index[intersect(person, semRole2),]$no=agent$index[intersect(person, semRole2),]$no + 1
					}
					...
					if('markerID'%in%names(meaning$external)){
						agent$flag$person[intersect(person, verbRole),]$yes=agent$flag$person[intersect(person, verbRole),]$yes + 1
						agent$flag$person[intersect(person, semRole2),]$yes=agent$flag$person[intersect(person, semRole2),]$yes + 1
						if(meaning$external$person==3){
							agent$flag$external[agent$flag$external$value==values,]$yes=agent$flag$external[agent$flag$external$value==values,]$yes+1
							if(semRole=='actor'){agent$flag$actor[agent$flag$actor$value==values,]$yes=agent$flag$actor[agent$flag$actor$value==values,]$yes+1}
							if(semRole=='undergoer'){agent$flag$undergoer[agent$flag$undergoer$value==values,]$yes=agent$flag$undergoer[agent$flag$undergoer$value==values,]$yes+1}
					}	}	
					if(!'markerID'%in%names(meaning$external)){
						agent$flag$person[intersect(person, verbRole),]$no=agent$flag$person[intersect(person, verbRole),]$no + 1
						agent$flag$person[intersect(person, semRole2),]$no=agent$flag$person[intersect(person, semRole2),]$no + 1				
						if(meaning$external$person==3){
							agent$flag$external[agent$flag$external$value==values,]$no=agent$flag$external[agent$flag$external$value==values,]$no+1
							if(semRole=='actor'){agent$flag$actor[agent$flag$actor$value==values,]$no=agent$flag$actor[agent$flag$actor$value==values,]$no+1}
							if(semRole=='undergoer'){agent$flag$undergoer[agent$flag$undergoer$value==values,]$no=agent$flag$undergoer[agent$flag$undergoer$value==values,]$no+1}
				}	}	}	
				if(meaning$verb$type=='twoPlace' & is.data.frame(meaning$internal)){
					verbRole=grep('internal',agent$index$role)
					person=grep(meaning$internal$person, agent$index$person)
					...
					if('markerID'%in%names(meaning$internal)){
						...
						if(meaning$internal$person==3){
							agent$flag$internal[agent$flag$internal$value==values,]$yes=agent$flag$internal[agent$flag$internal$value==values,]$yes+1
							if(semRole=='actor'){agent$flag$actor[agent$flag$actor$value==values,]$yes=agent$flag$actor[agent$flag$actor$value==values,]$yes+1}
							if(semRole=='undergoer'){agent$flag$undergoer[agent$flag$undergoer$value==values,]$yes=agent$flag$undergoer[agent$flag$undergoer$value==values,]$yes+1}
					}	}	
					if(!'markerID'%in%names(meaning$internal)){
						...
						agent$flag$person[intersect(person, semRole2),]$no=agent$flag$person[intersect(person, semRole2),]$no + 1				
						if(meaning$internal$person==3){
							agent$flag$internal[agent$flag$internal$value==values,]$no=agent$flag$internal[agent$flag$internal$value==values,]$no+1
							if(semRole=='actor'){agent$flag$actor[agent$flag$actor$value==values,]$no=agent$flag$actor[agent$flag$actor$value==values,]$no+1}
							if(semRole=='undergoer'){agent$flag$undergoer[agent$flag$undergoer$value==values,]$no=agent$flag$undergoer[agent$flag$undergoer$value==values,]$no+1}
			}	}	}	}
			if(meaning$verb$type=='twoPlace' & is.data.frame(meaning$internal)){
				wordOrder=names(meaning); wordOrder=wordOrder[wordOrder!='target']
				wordOrder=gsub('external','S',wordOrder); wordOrder=gsub('internal','O',wordOrder); wordOrder=gsub('verb','V',wordOrder)
				...
				agent$wordOrder[agent$wordOrder$order==wordOrder,]$oddsSuccess=agent$wordOrder[agent$wordOrder$order==wordOrder,]$success/(agent$wordOrder[agent$wordOrder$order==wordOrder,]$freq-agent$wordOrder[agent$wordOrder$order==wordOrder,]$success)
				...
				if(success==1){
					...
					agent$roleCorrelation[agent$roleCorrelation$role==macroRole,]$internal=agent$roleCorrelation[agent$roleCorrelation$role==macroRole,]$internal + 1
				...
				if(grep('internal',names(meaning))==1){
					agent$position[agent$position$role=='internal',]$first=agent$position[agent$position$role=='internal',]$first + 1 	
					agent$position[agent$position$role==macroRole,]$first=agent$position[agent$position$role==macroRole,]$first + 1 	
					if(success==1){
						agent$position[agent$position$role=='internal',]$success=agent$position[agent$position$role=='internal',]$success + 1 	
						agent$position[agent$position$role==macroRole,]$success=agent$position[agent$position$role==macroRole,]$success + 1 						
					}
					agent$position[agent$position$role=='internal',]$oddsSuccess=agent$position[agent$position$role=='internal',]$success/(agent$position[agent$position$role=='internal',]$first-agent$position[agent$position$role=='internal',]$success)
					agent$position[agent$position$role==macroRole,]$oddsSuccess=agent$position[agent$position$role==macroRole,]$success/(agent$position[agent$position$role==macroRole,]$first-agent$position[agent$position$role==macroRole,]$success)
				}
				if(success==1){
			...
			if(is.data.frame(meaning$external)){
				...
				if(success==1){
					...
					if(meaning$verb$type=='onePlace'){agent$usageHistory$nouns[nrow(agent$usageHistory$nouns),grep('D\\d',names(agent$usageHistory$nouns))]=meaning$target[, grep('A\\d',names(meaning$target))]} #cf. next condition
					...
					if(meaning$verb$type=='twoPlace'){ 		
						agent$roleCorrelation[agent$roleCorrelation$role==macroRole,]$external=agent$roleCorrelation[agent$roleCorrelation$role==macroRole,]$external + 1
						agent$usageHistory$nouns[nrow(agent$usageHistory$nouns),grep('D\\d',names(agent$usageHistory$nouns))]=meaning$target[, grep(paste(toupper(gsub('^(.).*','\\1', macroRole)),'\\d', sep=''),names(meaning$target))]
				}	}
				if(grep('external',names(meaning))==1){
					agent$position[agent$position$role=='external',]$first=agent$position[agent$position$role=='external',]$first + 1 	
					if(meaning$verb$type=='twoPlace'){agent$position[agent$position$role==macroRole,]$first=agent$position[agent$position$role==macroRole,]$first + 1} 	
					if(success==1){
						agent$position[agent$position$role=='external',]$success=agent$position[agent$position$role=='external',]$success + 1 	
						if(meaning$verb$type=='twoPlace'){agent$position[agent$position$role==macroRole,]$success=agent$position[agent$position$role==macroRole,]$success + 1} 						
					}
					agent$position[agent$position$role=='external',]$oddsSuccess=agent$position[agent$position$role=='external',]$success/(agent$position[agent$position$role=='external',]$first-agent$position[agent$position$role=='external',]$success)
					if(meaning$verb$type=='twoPlace'){agent$position[agent$position$role==macroRole,]$oddsSuccess=agent$position[agent$position$role==macroRole,]$success/(agent$position[agent$position$role==macroRole,]$first-agent$position[agent$position$role==macroRole,]$success)}
				}
				if(success==1){
...
PROCREATE=function(speakerID, hearerID){
	...
	if(population[[speakerID]]$fertile=='yes' & population[[hearerID]]$fertile=='yes'){
		if((population[[speakerID]]$age + population[[hearerID]]$age)/2 > (procreationAge*deathAge)){
			...
			offspring1$roleCorrelation=data.frame(role=c('actor','undergoer'),external=0, internal=0, stringsAsFactors=F); offspring2$roleCorrelation=offspring1$roleCorrelation
			offspring1$position=data.frame(role=c('external','internal', 'actor', 'undergoer'),first=0, success=0, oddsSuccess=0, stringsAsFactors=F); offspring2$position=offspring1$position
			offspring1$index=data.frame(role=rep(c('external','internal', 'actor', 'undergoer'), each=3), person=rep(1:3, 4), yes=0, no=4, stringsAsFactors=F); offspring2$index=offspring1$index	
			values=vector(); for(i in 1:length(distinctions)){values=c(values, seq(0,1,length.out=distinctions[i]))}
			offspring1$flag=list(
				person=data.frame(role=rep(c('external','internal', 'actor', 'undergoer'), each=3), person=rep(1:3, 4), yes=0, no=4, stringsAsFactors=F), 
				external=data.frame(dimension=rep(1:length(distinctions), distinctions), value=values, yes=0, no=4, stringsAsFactors=F),
				internal=data.frame(dimension=rep(1:length(distinctions), distinctions), value=values, yes=0, no=4, stringsAsFactors=F),
				actor=data.frame(dimension=rep(1:length(distinctions), distinctions), value=values, yes=0, no=4, stringsAsFactors=F),
			...
			offspring1$wordOrder=data.frame(order=c('SVO','SOV', 'VSO', 'VOS', 'OSV', 'OVS'),freq=0, success=0, oddsSuccess=0, stringsAsFactors=F); offspring2$wordOrder=offspring1$wordOrder
...
DIE=function(agentID){
	...
	if(agent$age > deathAge){
		...
		if(saveAll==F){
			#rm usage history and common ground
			graveyard$brains[[length(graveyard$brains)]] <<- graveyard$brains[[length(graveyard$brains)]][-c(7, 8)]	
...
TURN=function(speakerID, hearerID){
	...
	if(success==1){if(erosion==T){population[[hearerID]] <<- EROSION(hearerID, interpretation)}}
utterance
}

TALK=function(nTurns, output=T){
	talkAge=world$talkAge; desemanticization=world$desemanticization; semUpdateAge=world$semUpdateAge; procreationAge=world$procreationAge; deathAge=world$deathAge
...
RUN=function(nMinutes=1){
	nTurns=world$nTurns
	start=proc.time()[3]
	stop=proc.time()[3]
	while(((stop-start)/60)<nMinutes){
		TALK(sample(nTurns,1))
		stop=proc.time()[3]
}	}

MULTIRUN=function(nHours=36, name='lineage', rep=10){
	for(i in 1:rep){
		FOUND(2)
		RUN(nHours*60)
...
CHECKSETTINGS=function(){
	warning="If you have changed changed any of the default settings, you might have to create a new population first (using FOUND()) before the changes apply."
	...
	if(!is.numeric(world$world$oddsNewExternal)){warning[length(warning)+1]="Odds for new external participant should be numeric"}
	if(!is.numeric(world$world$oddsNewInternal)){warning[length(warning)+1]="Odds for new internal participant should be numeric"}
...
#	world$frequencyImpact=1/10,	#if candidateOrdering=='all2', how should frequency be weighed with respect to match? impact=1:equally, impact=below 1: impact times less important, impact=above 1: impact times more important.
#	world$collostructionImpact=1/100,	#if candidateOrdering=='all' and for finding index referent, how should collostruction frequency be weighed with respect to match? impact=1:equally, impact=below 1: impact times less important, impact=above 1: impact times more important.
#	world$recencyImpact=1/10, 	#if candidateOrdering=='all', how should recency be weighed with respect to match? impact=1:equally, impact=below 1: impact times less important, impact=above 1: impact times more important.
...
#	world$agentFirst=F, 	#is AgentFirst used as a strategy?
#	world$topicFirst=F, #should topic be put in first position?
